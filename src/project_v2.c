...
void projectV2(const char * i_file, const char * o_file, unsigned long nb_split)
{
	...
	fprintf(stderr,
		"Projet version 2 version with %lu split of %lu lines\n",
		nb_split,
		nb_lines_per_files);
...
{
	pid_t *childList = (pid_t*)malloc(sizeof(pid_t) * nb_split);
...
void projectV2_combMerge(unsigned long nb_split, const char ** filenames_sort, const char * o_file)
{
	...
	unsigned long end = (nb_split / 2) + 1;
	...
	if (childPids[0] == 0 || childPids[1] == 0)
	{
		...
		nb_print = snprintf(current_name,
			PROJECT_FILENAME_MAX_SIZE,
			"/tmp/final_split_%d_merge.txt", getpid());
		...
		SU_removeFile(previous_name);
		SU_removeFile(filenames_sort[end - 1]);
	...
	else
	{
		...
		nb_print = snprintf(previous_name,
			PROJECT_FILENAME_MAX_SIZE,
			"/tmp/final_split_%d_merge.txt", childPids[0]);
